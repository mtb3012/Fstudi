<?php
    ...
    function product_add($name,$image,$price,$sale,$category,
        $quantity,$description,$hot,$status){
        $sql="INSERT INTO SanPham(TenSanPham,HinhAnh,Gia,GiaKhuyenMai,MaDanhMuc,SoLuong,MoTa,Hot,TrangThai)
        VALUES (?,?,?,?,?, ?,?,?,?)";
        return pdo_query($sql,$name,$image,$price,$sale,$category,
        $quantity,$description,$hot,$status);
    }
    ...
    function product_edit($id,$name,$image,$price,$sale,$category,
        $quantity,$description,$hot,$status){
        $sql="UPDATE SanPham SET TenSanPham=? ,HinhAnh=? ,Gia=? ,GiaKhuyenMai=? ,MaDanhMuc=? ,SoLuong=? ,
        MoTa=? ,Hot=? ,TrangThai=? WHERE MaSanPham =? ";
        return pdo_execute($sql,$name,$image,$price,$sale,$category,
        $quantity,$description,$hot,$status,$id);
    }
